...
FlowSmith is a workflow builder for operations teams, automation builders, AI workflow engineers. The product must support the complete path from setup to reviewed output, with enough traceability and operational control for real use by a small team.

## 2. Goals

- Deliver one reliable end-to-end workflow for workflow builder.
...
- visual workflow graph, step templates, condition rules, dry-run simulator, execution logs
...
- The product must provide a guided intake form for workflow builder.
...
- The product should support at least one integration handoff in MVP: MCP tools, webhooks, queues, CRMs, databases.
...
Primary risk areas: runaway automations, partial failures, weak retry design, hidden data movement.
